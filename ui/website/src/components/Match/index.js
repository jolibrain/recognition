...
const mapStateToProps = (state, ownProps = {}) => {
  ...
  if(state.matches.length > 0) {
    ...
    const matchingIndex = state.matches.filter(item => item.input.meta.TPX).findIndex(item => item.input.img.indexOf(matchId) != -1);

    props.item = state.matches[matchingIndex];

    if(!ownProps.params || !ownProps.params.follower)
      props.followingMatches = state.matches.filter(item => item.input.meta.TPX).slice(matchingIndex + 1);
